...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['okul_no']) && isset($_POST['sifre'])) {
        $okul_no = mysqli_real_escape_string($conn, $_POST['okul_no']);
        $sifre = $_POST['sifre'];

        // Kullanıcıyı veritabanından al
        $sql = "SELECT * FROM ogrenciler WHERE okul_no = ?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "s", $okul_no);
        ...
        if ($result && $result->num_rows > 0) {
            ...
            // Şifreyi kontrol et
            if (password_verify($sifre, $ogrenci['sifre'])) {
                $_SESSION['ogrenci_id'] = $ogrenci['ogrenci_id'];
        ...
        $error = "Kullanıcı adı veya şifre alanı boş.";
    }
}
$conn->close();
?>
...
<head>
    ...
    <title>Öğrenci Girişi - Okul Bilgi Sistemi</title>
...
<body>
    <div class="login-container">
        ...
        <form action="ogrencı_login.php" method="post">
            <div class="form-group">
                <label for="okul_no" class="form-label">OKUL NUMARASI</label>
                <input type="text" class="form-control" id="okul_no" name="okul_no" required 
